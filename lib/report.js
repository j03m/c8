...
const { isAbsolute, resolve, extname, join, dirname } = require('path')
...
class Report {
    ...
    for (const v8ScriptCov of v8ProcessCov.result) {
      if (/^file:\/\//.test(v8ScriptCov.url)) {
        try {
          ...
          // indicate to --all that this src file is accounted for
      ...
      // important: as of c8 8.0.0 --include and --exclude are applied to src files
      // if src files are present.
      const filterTarget = this._getSourceMappedFileIfPresent(v8ScriptCov.url)
      // indicate to --all that this src file is accounted for in case it differs
      // from the executable
      fileIndex.add(filterTarget)
      if (this.exclude.shouldInstrument(filterTarget) &&
        (!this.omitRelative || isAbsolute(filterTarget))) {
  ...
  _getSourceMappedFileIfPresent (url) {
    if (existsSync(url) && !url.includes('node_modules')) {
      const sourceMapPath = getSourceMapFromFile(url)
      if (sourceMapPath && existsSync(sourceMapPath)) {
        const sourceMap = JSON.parse(readFileSync(sourceMapPath).toString())
        return this._lookupSrcFile(sourceMap, url)
      }
    }
    return url
  }
  ...
  /**
   * In order to target src files with globs, here if a v8 coverage json entry contains a source map, we can easily
   * look up and decode the src file location. As such, this will allow user supplied globs to target source files.
   * @param v8ProcessCov
   * @param lookupUrl executable file in question
   * @returns string path to the src or the original url
   * @private
   */
  // Todo: This need to go in a util file and get its own unit tests
  _lookupSrcFile (sourceMap, lookupUrl) {
    const sourceRoot = sourceMap.sourceRoot ? sourceMap.sourceRoot.replace('file://', '') : ''
    const sourcePath = sourceMap.sources.length >= 1 ? sourceMap.sources[0].replace('file://', '') : sourceMap.file
    const candidatePath = join(sourceRoot, sourcePath)

    return isAbsolute(candidatePath)
      ? candidatePath
      : resolve(dirname(lookupUrl), candidatePath)
  }
}
